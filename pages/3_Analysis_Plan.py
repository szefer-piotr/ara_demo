...
# ────────────────────────── helpers ──────────────────────────
def first_chunk(chunks: List[Dict[str, str]], _type: str, default: str = "") -> str:
...
if not hypo["plan_accepted"]:
    ...
    if prompt:
        ...
        chunks = mock_llm(prompt, history=[{"type": "text", "content": m} for _, m in plan_chat])
        reply = first_chunk(chunks, "text", "[No response]")
        plan_chat.append(("assistant", reply))
        st.chat_message("assistant").write(reply)

        # optional: store code chunk as run under a synthetic plan-level list
...
with main:
    ...
    # action buttons Depending on finished
    if step["finished"]:
    ...
    if not step["finished"]:
        ...
        if prompt:
            ...
            chunks = mock_llm(prompt, history=[{"type": "text", "content": m} for _, m in step["chat_history"]])
