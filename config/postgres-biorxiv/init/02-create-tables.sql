...
CREATE TABLE IF NOT EXISTS pdf_processing_state (
    ...
    pdf_uid VARCHAR(500) UNIQUE NOT NULL,
...
CREATE TABLE IF NOT EXISTS extracted_texts (
    ...
    FOREIGN KEY (pdf_uid) REFERENCES pdf_processing_state(pdf_uid)
);

-- Text chunks for RAG processing
CREATE TABLE IF NOT EXISTS text_chunks (
    id SERIAL PRIMARY KEY,
    pdf_uid VARCHAR(500) NOT NULL,
...
CREATE TABLE gemini_responses (
    id SERIAL PRIMARY KEY,
    pdf_uid VARCHAR(500) REFERENCES pdf_processing_state(pdf_uid),
    s3_key VARCHAR(255),
...
CREATE INDEX idx_gemini_responses_pdf_uid ON gemini_responses(pdf_uid);
...
CREATE INDEX IF NOT EXISTS idx_text_chunks_pdf_uid ON text_chunks(pdf_uid);
...
CREATE INDEX IF NOT EXISTS idx_processing_logs_pdf_uid ON processing_logs(pdf_uid);
